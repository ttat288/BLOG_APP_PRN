...
namespace BlogWinApp
{
    public partial class frmEditor : Form
    {
        ...
        frmNewBlog parentForm;
        public frmEditor(PostTbl Post, frmNewBlog ParentFrm)
        {
            InitializeComponent();
            post = Post;
        ...
        {
            richTextBox1.SaveFile($"../../../../SECRET/Posts/{post.PostId}.doc");
            crPost = postRepository.CreatePost(post);
            if (crPost)
            {
                MessageBox.Show("Bài viết đã được publish!");
                parentForm.ResetFormControls();
            }
        ...
        {
            MessageBox.Show("Bài viết đã được hủy!");
            this.Close();
        }
    }
}
